...
public class SubjectsCategorizerTest extends TestCase {
    ...
    public void testPretrainedTopicsModel() throws IOException,
        ...
        Categorizer categorizer = factory.loadInstance(
                "models/topics-51-tfidf-65536-model.gz", true);

        String query = "The Sydney Opera House was conceived and"
                + " largely built by Jørn Utzon.";
        List<String> suggestions = categorizer.guessCategories(query, 3, 5.0);
        assertEquals(1, suggestions.size());
        assertEquals("architecture", suggestions.get(0));

        query = "The database vendor Oracle bought the software editor"
                + " and hardware manufacturer Sun in 2009.";
        ...
        query = "The financial crisis of 2007–2010 has been called by leading"
                + " economists the worst financial crisis since the"
